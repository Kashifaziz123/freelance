...
class PrintInvoiceSummary(models.TransientModel):
    ...
    def action_print_invoice_summary(self):
        ...
        for wizard in self:
            ...
            worksheet2.write_merge(0, 0, 0, 2, heading, easyxf('font:height 200; align: horiz center;pattern: pattern solid, fore_color black; font: color white; font:bold True;' "borders: top thin,bottom thin"))
            if wizard.invoice_status == 'all':
                invoice_objs = self.env['account.invoice'].search([('date_invoice','>=',wizard.from_date),
                                                               ('date_invoice','<=',wizard.to_date),('type','=','out_invoice')])
            ...
            for invoice in invoice_objs:
                ...
                amount_tot += amount
                row += 1
                key = str(invoice.partner_id.name)+'_'+ str(invoice.currency_id.name)
                if key not in customer_payment_data:
                    customer_payment_data.update({key: invoice.amount_total})
                else:
                    paid_amount_data = customer_payment_data[key] + invoice.amount_total
                    customer_payment_data.update({key: paid_amount_data})
